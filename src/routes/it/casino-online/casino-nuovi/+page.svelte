<script lang="ts">
	...
	import { resolveCasinoIndexPath, resolveNewCasinoPath } from '$lib/link-resolver';
	import type { RouteId } from '$app/types';
	import type { NewCasinosPageData } from './+page.server';
	import AuthorBox from '../../../../component/author/author-box.svelte';

	let { data }: { data: { page: NewCasinosPageData } } = $props();
</script>
...
<Breadcrumbs
	breadcrumbs={[
		...
		{
			route: { id: resolveNewCasinoPath(CountryCodes.it) as RouteId },
			title: 'Gioca nei nuovi Casino Online in Italia',
			label: 'Nuovi Casino'
		}
	]}
/>
...
<h1 class="page-title">
	I nuovi casino online in Italia {new Date().getFullYear()}: recensioni, bonus e giochi
</h1>
