			...
			},
			{
				...
				categories: [
					...
					},
					{
						name: 'orderBy',
						label: 'Ordenar por',
						filters: [{ title: 'Más Jugadas', value: 'sessions:desc' }] // we set the categories of the currently applied filters to the slugs of the features of the current slot category, this will allow us to keep track of which categories of filters are currently applied and to update the currently applied filters accordingly when the user applies or removes filters
					}
				]
			}
		);
...
<Breadcrumbs
	breadcrumbs={[
		...
		{
			route: { id: '/es/slot-gratis/slot-piu-giocate' },
