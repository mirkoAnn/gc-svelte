<script lang="ts">
  ...
  import PointsLoader from "./../../../graphics/loader/points-loader.svelte";
...
<div class="navbar-search-results">
  {#if navSearchManager.isResultsListEmpty()}
    ...
    <p class="search-results-message">
      Nessun risultato trovato per "{navSearchManager.getSearchInput()}"
    </p>
    ...
    <div class="navbar-search-results-container">
      {#if navSearchManager.getSearchResults().casinosIds.length > 0}
        <div class="results-category-container">
          <h3 class="results-category-title">Casinos</h3>
          <CasinoCards
      ...
      {#if navSearchManager.getSearchResults().slots.length > 0}
        <div class="results-category-container">
          <h3 class="results-category-title">Tragaperras</h3>
          <GamesGallery
      ...
      {/if}
      {#if navSearchManager.getSearchResults().crash.length > 0}
        <div class="results-category-container">
          <h3 class="results-category-title">Giochi Crash</h3>
          <!-- <GamesCarousel games={navSearchManager.getSearchResults().crash} category="slots" /> -->
        </div>
      {/if}
      {#if navSearchManager.getSearchResults().bingo.length > 0}
        <div class="results-category-container">
          <h3 class="results-category-title">Bingo</h3>
          <!-- <GamesCarousel games={navSearchManager.getSearchResults().bingo} category="slots" /> -->
        </div>
      {/if}
      {#if navSearchManager.getSearchResults().roulettes.length > 0}
        <div class="results-category-container">
          <h3 class="results-category-title">Roulette</h3>
          <!-- <GamesCarousel
      ...
      {#if navSearchManager.getSearchResults().providers.length > 0}
        <div class="results-category-container">
          <h3 class="results-category-title">Providers</h3>
          <ProvidersGallery
            providers={navSearchManager.getSearchResults().providers}
          />
        </div>
      {/if}
      {#if navSearchManager.getSearchResults().faqs.length > 0}
        <div class="results-category-container">
          <h3 class="results-category-title">Domande Frequenti</h3>
          <ul>
            {#each navSearchManager.getSearchResults().faqs as faq}
              <li><a href="/faq/{faq.title}">{faq.title}</a></li>
            {/each}
          </ul>
        </div>
      {/if}
