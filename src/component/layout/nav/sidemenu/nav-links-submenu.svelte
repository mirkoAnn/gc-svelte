<script lang="ts">
  ...
  let { menu, menuIndex }: { menu: any; menuIndex: number } = $props();
</script>
...
>
  <ul
    ...
    role="list"
    aria-label="Sottomenu delle sezioni principali del sito"
  >
    {#each menu.submenuItems as subitem}
      <li class="nav-links-submenu-item">
        ...
        <a
          ...
          href={`/${appManager.getCountryCode() + subitem.href}`}
        >
          {subitem.label}
        </a>
