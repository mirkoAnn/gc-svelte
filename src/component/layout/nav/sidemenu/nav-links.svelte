...
<div class="nav-links-menus-outer">
  {#each navManager.getMenuItems() as menuItem}
    <ul
      id="nav-links-menu-{menuItem.section}"
      class="nav-links-menu nav-links-menu-{menuItem.section} {menuItem.section ===
      initialSection
        ? 'initial-section'
        : ''}"
      role="tablist"
      aria-label="Questi bottono permettono di mostrare i menu relativi alle sezioni principali del sito"
      style="--nav-submenu-links-bg-color: var(--nav-submenu-links-bg-color-{menuItem.section});"
    >
      {#each menuItem.items as item, i}
        <li
          class="nav-links-menu-item nav-links-menu-item-{i}"
          role="presentation"
        >
          <button
