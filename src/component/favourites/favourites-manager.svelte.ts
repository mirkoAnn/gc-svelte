...
let favouritesList: any = $state({
  slots: [],
});

export const favouritesManager = {
  initialize: (newList: any) => {
  ...
  toggleFromList: (data: any, category: string) => {
    switch (category) {
      case "slot":
        const index = favouritesList.slots.findIndex((slot: any) => {
          return slot.id === data.id;
        });
        if (index < 0) {
          favouritesList.slots = [...favouritesList.slots, data];
        } else {
